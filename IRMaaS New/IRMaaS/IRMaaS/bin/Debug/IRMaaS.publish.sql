...
CREATE TABLE [stg].[CISCO_SECUR_RISK_ASES_STG] (
    [RISK_NBR]               INT            NOT NULL,
    [RISK_NM]                VARCHAR (255)  NOT NULL,
    [CTL_EFF]                VARCHAR (150)  NULL,
    [RISK_LVL_IMP_CODE]      INT            NULL,
    [RISK_LVL_IMP_DESC]      VARCHAR (150)  NOT NULL,
    [RISK_LVL_PRBL_CODE]     INT            NOT NULL,
    [RISK_LVL_PRBL_DESC]     VARCHAR (150)  NOT NULL,
    [RISK_LVL_OVALL]         VARCHAR (150)  NOT NULL,
    [RISK_DESC]              TEXT           NULL,
    [IMP_COMMT]              TEXT           NULL,
...
GO
ALTER TABLE [dbo].[CLNT_VULN_SECUR_CTL]
    ADD DEFAULT ((0)) FOR [PRI_CTL_IND];
...
GO
ALTER TABLE [dbo].[CLNT_VULN_INSTC] WITH NOCHECK
    ADD FOREIGN KEY ([OS_KEY]) REFERENCES [dbo].[OS] ([OS_KEY]);
...
GO
PRINT N'Creating unnamed constraint on [dbo].[OS]...';


GO
ALTER TABLE [dbo].[OS] WITH NOCHECK
    ADD FOREIGN KEY ([OS_CATGY_KEY]) REFERENCES [dbo].[OS_CATGY] ([OS_CATGY_KEY]);
...
SET NOCOUNT ON
	...
	SELECT  A.CLNT_VULN_INSTC_KEY,A.SECUR_CTL_CD,A.CREAT_DT,A.UPDT_DT,A.UPDT_USER_ID,
			B.REG_CMPLN_CD, B.SECUR_CTL_FAM_CD,B.SECUR_CTL_FAM_NM,B.SECUR_OBJ_CD,
			B.SECUR_OBJ_NM,B.SECUR_CTL_NM,B.SECUR_CTL_DESC,A.PRI_CTL_IND 
	FROM	CLNT_VULN_SECUR_CTL		A
	JOIN	SECUR_CTL				B
	ON		A.SECUR_CTL_CD		=	B.SECUR_CTL_CD
	WHERE	CLNT_VULN_INSTC_KEY	=	@CLNT_VULN_INSTC_KEY
	AND		A.ROW_STS_KEY		= 1
...
SET NOCOUNT ON
              ...
              SELECT DISTINCT A.SECUR_CTL_MAP_KEY,A.SEC_REG_CMPLN_CD [REG_CMPLN_CD],
                     C.REG_CMPLN_NM,A.SEC_SECUR_CTL_CD [SECUR_CTL_CD],B.SECUR_CTL_NM,
...
BEGIN
	SELECT  DISTINCT H.ORG_NM [Client NAME],B.CLNT_ORG_KEY, A.CLNT_ENGMT_CD,B.CLNT_ENGMT_NM,D.SECUR_SRVC_NM,D.SECUR_SRVC_CD,
			...
			A.SRVC_ENGMT_STS_KEY,B.ENGMT_STS_KEY,I.LKP_ENTY_NM ReviewStatus
...
SET NOCOUNT ON
		...
		FROM VULN_SEV 
		WHERE @BaseScore >= CVSS_BAS_SCOR_MIN 
		ORDER BY CVSS_BAS_SCOR_MIN DESC
...
SET NOCOUNT ON
			...
			SELECT  A.CLNT_VULN_INSTC_KEY,A.VULN_NM,A.VULN_SRC_KEY,B.LKP_ENTY_DESC VULN_SRC,A.VULN_SEV_CD,C.VULN_SEV_NM,A.IPADR,
			...
			AND		A.CLNT_VULN_INSTC_KEY	=   @CLNT_VULN_INSTC_KEY
END TRY
...
SET NOCOUNT ON
			...
			SELECT  A.CLNT_VULN_INSTC_KEY,A.VULN_NM,CONVERT(VARCHAR,A.CREAT_DT,101)CREAT_DT,A.VULN_DESC,A.VULN_TECH_COMMT_TXT,
					A.VULN_IMP_COMMT_TXT,A.RECOM_COMMT_TXT,A.ROOT_CAUS_COMMT_TXT,A.ROOT_CAUS_ANLYS_CD,
					B.VULN_INSTC_STS_CD,B.VULN_INSTC_STS_NM,D.CVE_ID,D.CVE_DESC,D.LST_MOD_DT,D.CWE_ID,
					A.VULN_BAS_SCOR,A.VULN_TMPRL_SCOR,A.VULN_ENV_SCOR,A.VULN_VCTR_TXT,
					F.VULN_SEV_NM,G.RISK_PRBL_NM,E.VULN_IMP_NM,H.RMDTN_CST_EFFRT_NM,J.SECUR_CTL_FAM_NM,K.SECUR_OBJ_NM,K.SECUR_CTL_NM
			FROM	CLNT_VULN_INSTC				A
			LEFT JOIN	VULN_INSTC_STS				B
			ON		A.VULN_INSTC_STS_CD		=   B.VULN_INSTC_STS_CD
			LEFT JOIN	ROOT_CAUS_ANLYS				C
			ON		A.ROOT_CAUS_ANLYS_CD	=	C.ROOT_CAUS_ANLYS_CD
			LEFT JOIN	CVE							D
			ON		A.CVE_ID				=   D.CVE_ID
			LEFT JOIN	VULN_IMP					E
			ON		A.VULN_IMP_CD			=   E.VULN_IMP_CD
			LEFT JOIN	VULN_SEV					F
			ON		A.VULN_SEV_CD			=   F.VULN_SEV_CD
			LEFT JOIN	RISK_PRBL					G
			ON		A.RISK_PRBL_CD			=	G.RISK_PRBL_CD
			LEFT JOIN	RMDTN_CST_EFFRT				H
			ON		A.RMDTN_CST_EFFRT_CD	=	H.RMDTN_CST_EFFRT_CD
			LEFT JOIN	CVE_SECUR_CTL_MAP			I
			ON		A.CVE_ID				=   I.CVE_ID
			LEFT JOIN	SECUR_CTL					J
			ON		I.SECUR_CTL_CD			=	J.SECUR_CTL_CD
			LEFT JOIN	SECUR_CTL					K
			ON		J.SECUR_CTL_FAM_CD		=	K.SECUR_CTL_FAM_CD
			WHERE	A.ROW_STS_KEY			=   1
			AND		A.CLNT_VULN_INSTC_KEY	=   @CLNT_VULN_INSTC_KEY
END TRY
...
SET NOCOUNT ON
 ...
 INSERT CLNT_VULN_INSTC (ROW_STS_KEY,ORG_KEY, CLNT_ENGMT_CD, SECUR_SRVC_CD,VULN_SRC_KEY,VULN_INSTC_STS_CD,VULN_SEV_CD,VULN_IMP_CD,
 RISK_PRBL_CD,RMDTN_CST_EFFRT_CD, ROOT_CAUS_ANLYS_CD,CVE_ID,SRC_VULN_INSTC_ID,
 ...
 CREAT_DT,CREAT_USER_ID) 
 VALUES (@ROW_STS_KEY,@ORG_KEY,@CLNT_ENGMT_CD,@SECUR_SRVC_CD,@VULN_SRC_KEY,@VULN_INSTC_STS_CD,@VULN_SEV_CD,
 @VULN_IMP_CD,@RISK_PRBL_CD,@RMDTN_CST_EFFRT_CD, @ROOT_CAUS_ANLYS_CD,@CVE_ID,@SRC_VULN_INSTC_ID,
 @VULN_NM,@VULN_DESC,GETDATE(),@IPADR,@PORT_NBR,@SRC_ADVS_TXT,@VULN_BAS_SCOR,@VULN_IMP_SUB_SCOR,@VULN_EXPLT_SUB_SCOR,
 @VULN_TMPRL_SCOR,@VULN_ENV_SCOR,@VULN_VCTR_TXT,@NTWK_NM,@PRTCL_NM,@HST_NM,@DOM_NM,@SFTW_NM,@OS_KEY,@APPL_URL,@NETBIOS_NM,
 @MAC_ADR_NM,@VULN_TECH_COMMT_TXT,@VULN_IMP_COMMT_TXT,@RECOM_COMMT_TXT,@ROOT_CAUS_COMMT_TXT,
 GETDATE(),@USER_ID)
...
SET NOCOUNT ON
	...
	BEGIN
		 ...
		 DELETE FROM CLNT_VULN_SECUR_CTL
		 WHERE	CLNT_VULN_INSTC_KEY	=	@CLNT_VULN_INSTC_KEY
	...
	IF @Flag = 'I'
	BEGIN
		 INSERT CLNT_VULN_SECUR_CTL(CLNT_VULN_INSTC_KEY,REG_CMPLN_CD,REG_CMPLN_VER,SECUR_CTL_CD,CREAT_DT,
				CREAT_USER_ID,ROW_STS_KEY,PRI_CTL_IND) 
		 VALUES (@CLNT_VULN_INSTC_KEY,@REG_CMPLN_CD,@REG_CMPLN_VER,@SECUR_CTL_CD,GETDATE(),@USER_ID,@ROW_STS_KEY,@PRI_CTL_IND)
...
END
GO
PRINT N'Creating [dbo].[Analyst_ListRootCause]...';
...
SET NOCOUNT ON
			...
			SELECT  ROOT_CAUS_ANLYS_CD,ROOT_CAUS_ANLYS_NM
...
SET NOCOUNT ON
			...
			SELECT  DISTINCT A.CLNT_VULN_INSTC_KEY,A.VULN_NM,A.VULN_SRC_KEY,B.LKP_ENTY_NM VULN_SRC,A.VULN_SEV_CD,G.VULN_SEV_NM,A.IPADR,
...
SET NOCOUNT ON
			...
			SELECT A.ROW_STS_KEY,A.ORG_KEY, A.CLNT_ENGMT_CD, A.SECUR_SRVC_CD,A.VULN_SRC_KEY,A.VULN_INSTC_STS_CD,L.VULN_INSTC_STS_NM, 
			A.VULN_SEV_CD,M.VULN_SEV_NM,A.VULN_IMP_CD,
			A.RISK_PRBL_CD,A.RMDTN_CST_EFFRT_CD, A.ROOT_CAUS_ANLYS_CD,A.CVE_ID,A.SRC_VULN_INSTC_ID,
			A.VULN_NM,A.VULN_DESC,A.VULN_CREAT_DT,A.IPADR,PORT_NBR,A.SRC_ADVS_TXT,A.VULN_BAS_SCOR,VULN_IMP_SUB_SCOR,VULN_EXPLT_SUB_SCOR,
			VULN_TMPRL_SCOR,VULN_ENV_SCOR,VULN_VCTR_TXT,NTWK_NM,PRTCL_NM,HST_NM,DOM_NM,SFTW_NM,ISNULL(E.OS_NM,'') OS_NM,APPL_URL,NETBIOS_NM,
			MAC_ADR_NM,VULN_TECH_COMMT_TXT,VULN_IMP_COMMT_TXT,RECOM_COMMT_TXT,ROOT_CAUS_COMMT_TXT,C.SECUR_SRVC_NM,
			A.SRC_VULN_SCAN_ID,A.SRC_VULN_SCAN_STRT_DT,A.SRC_VULN_SCAN_END_DT,F.LKP_ENTY_NM [Source Name],K.ROOT_CAUS_ANLYS_NM [RootCauseName],
			G.RISK_PRBL_NM,H.VULN_IMP_NM,I.RMDTN_CST_EFFRT_NM,J.CWE_ID,J.UPDT_DT,J.CVE_DESC,
			A.CREAT_DT,A.CREAT_USER_ID
			FROM	CLNT_VULN_INSTC							A
			JOIN	CLNT_SECUR_SRVC_ENGMT		    		B
			ON		A.CLNT_ENGMT_CD		=	B.CLNT_ENGMT_CD
			AND		A.SECUR_SRVC_CD		=   B.SECUR_SRVC_CD
			JOIN	SECUR_SRVC						C
			ON		A.SECUR_SRVC_CD		=   C.SECUR_SRVC_CD
			LEFT JOIN	VULN_INSTC_STS							L
			ON		A.VULN_INSTC_STS_CD	=	L.VULN_INSTC_STS_CD
			LEFT JOIN	VULN_SEV								M
			ON		A.VULN_SEV_CD		=	M.VULN_SEV_CD
			LEFT JOIN	OS									E
			ON		A.OS_KEY			=	E.OS_KEY
			LEFT JOIN	MSTR_LKP				     		F
			ON		F.MSTR_LKP_KEY		=   A.VULN_SRC_KEY
			LEFT JOIN	RISK_PRBL							G
			ON		G.RISK_PRBL_CD		=   A.RISK_PRBL_CD
			LEFT JOIN	VULN_IMP							H
			ON		H.VULN_IMP_CD		=	A.VULN_IMP_CD
			LEFT JOIN	RMDTN_CST_EFFRT						I
			ON		I.RMDTN_CST_EFFRT_CD =  A.RMDTN_CST_EFFRT_CD
			LEFT JOIN	CVE									J
			ON		J.CVE_ID			=	A.CVE_ID
			LEFT JOIN   ROOT_CAUS_ANLYS						K
			ON		K.ROOT_CAUS_ANLYS_CD	=  A.ROOT_CAUS_ANLYS_CD
			WHERE CLNT_VULN_INSTC_KEY = @CLNT_VULN_INSTC_KEY
...
SET NOCOUNT ON
		...
		UPDATE	CLNT_VULN_INSTC
		SET		
				VULN_INSTC_STS_CD	=	@VULN_INSTC_STS_CD,
				VULN_SEV_CD			=	@VULN_SEV_CD,
				VULN_IMP_CD			=	@VULN_IMP_CD,
				RISK_PRBL_CD		=	@RISK_PRBL_CD,
				RMDTN_CST_EFFRT_CD	=	@RMDTN_CST_EFFRT_CD,
				ROOT_CAUS_ANLYS_CD	=	@ROOT_CAUS_ANLYS_CD,
				CVE_ID				=	@CVE_ID,
				VULN_NM				=	@VULN_NM,
				VULN_DESC			=	@VULN_DESC,
				SRC_ADVS_TXT		=	@SRC_ADVS_TXT,
				VULN_BAS_SCOR		=	@VULN_BAS_SCOR,
				VULN_IMP_SUB_SCOR	=	@VULN_IMP_SUB_SCOR,
				VULN_EXPLT_SUB_SCOR =   @VULN_EXPLT_SUB_SCOR,
				VULN_TMPRL_SCOR		=	@VULN_TMPRL_SCOR,
				VULN_ENV_SCOR		=	@VULN_ENV_SCOR,
				VULN_VCTR_TXT		=	@VULN_VCTR_TXT,
				VULN_TECH_COMMT_TXT =   @VULN_TECH_COMMT_TXT,
				VULN_IMP_COMMT_TXT	=	@VULN_IMP_COMMT_TXT,
				RECOM_COMMT_TXT		=	@RECOM_COMMT_TXT,
				ROOT_CAUS_COMMT_TXT =	@ROOT_CAUS_COMMT_TXT,   
				UPDT_DT				 =	GETDATE(),
...
SET NOCOUNT ON
	...
	SELECT      A.CLNT_ENGMT_CD, A.SECUR_SRVC_CD, SRVC_ENGMT_STS_KEY, B.SECUR_SRVC_NM, A.ROW_STS_KEY, 
				...
				COUNT(C.APPL_FL_UPLOAD_LOG_KEY) FileCount,C.FL_SZ
	...
	GROUP BY	A.CLNT_ENGMT_CD, A.SECUR_SRVC_CD, SRVC_ENGMT_STS_KEY, B.SECUR_SRVC_NM, A.ROW_STS_KEY, 
				SRVC_EST_STRT_DT, SRVC_EST_END_DT, A.CREAT_DT, A.CREAT_USER_ID, A.UPDT_DT, A.UPDT_USER_ID,C.FL_SZ
...
SELECT  A.ORG_KEY,A.ROW_STS_KEY,[dbo].[fnGetMasterLkpNameByID](A.ROW_STS_KEY) OrgStatus,  
  ...
  A.PST_CD,A.ORG_DESC,A.STS_COMMT_TXT ,ISNULL([dbo].[fnGetParentOrgNameID](A.PAR_ORG_KEY),'') PAR_ORG_NM,@LiCOUNT UserCount 
FROM    ORG      A  
JOIN MSTR_LKP    B  
...
SELECT  A.FL_SZ,A.FL_STS_KEY,A.CLNT_ENGMT_CD,A.SECUR_SRVC_CD,B.ORG_NM
FROM    APPL_FL_UPLOAD_LOG	A
JOIN	ORG					B
ON		A.ORG_KEY		=		B.ORG_KEY
WHERE   FL_NM			=		@FL_NM
...
END;
GO
PRINT N'Creating [dbo].[MERGE_CLNT_VULN_INSTC_CISCO_PEN_TST]...';
...
/*
EXEC [dbo].[MERGE_CLNT_VULN_INSTC_CISCO_PEN_TST] ? OUTPUT , ? OUTPUT , ? OUTPUT , ? OUTPUT
...
CREATE PROCEDURE [dbo].[MERGE_CLNT_VULN_INSTC_CISCO_PEN_TST]
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT
			...
			,NULL AS DOM_NM
			,NULL AS SFTW_NM
			,pentst_stg.OS AS OS_NM
			,pentst_stg.URL AS APPL_URL
			,NULL AS NETBIOS_NM
			,NULL AS MAC_ADR_NM
			,pentst_stg.DTL AS VULN_TECH_COMMT_TXT
			,pentst_stg.SECUR_IMP AS VULN_IMP_COMMT_TXT
			,pentst_stg.MITG_FCT AS RECOM_COMMT_TXT
			,NULL AS ROOT_CAUS_COMMT_TXT
			,pentst_stg.APPL_FL_UPLOAD_LOG_KEY AS APPL_FL_UPLOAD_LOG_KEY

		FROM stg.CISCO_PEN_TST_STG pentst_stg
		...
		/*
		--VALIDATIONS --DELETE
		SELECT * FROM stg.CISCO_PEN_TST_STG
		SELECT DISTINCT CLNT_ENGMT_CD FROM #Vulnerability_PenTst
		SELECT * FROM #Vulnerability_PenTst WHERE CLNT_ENGMT_CD = 'QCL-TR-20160628' AND SRC_VULN_ID = '18e23d56-c966-11e5-939a-00155d700b07'
		SELECT * FROM #Vulnerability_PenTst WHERE CLNT_ENGMT_CD = 'QCL-TR-20160629'

		SELECT * FROM dbo.CLNT_SECUR_SRVC_ENGMT
		SELECT * FROM dbo.APPL_FL_UPLOAD_LOG
		
		SELECT * FROM #Vulnerability_PenTst WHERE CLNT_ENGMT_CD = 'QCL-TR-20160629'
		SELECT * FROM dbo.CLNT_VULN_INSTC WHERE CLNT_ENGMT_CD = 'QCL-TR-20160629'

		SELECT * FROM #Vulnerability_PenTst
		SELECT * FROM [dbo].[CVE] WHERE CVE_ID = 'CVE-1999-0001'
		*/
		...
		WHEN MATCHED AND 
			(			
			 ISNULL(_target.SRC_VULN_SCAN_ID,0)=ISNULL(_source.SRC_VULN_SCAN_ID,0)
			 AND _target.SRC_VULN_ID=_source.SRC_VULN_ID
			 AND _target.APPL_FL_UPLOAD_LOG_KEY=_source.APPL_FL_UPLOAD_LOG_KEY
			)
		THEN UPDATE SET 
			--_target.ROW_STS_KEY=_source.ROW_STS_KEY
			--,  _target.ORG_KEY=_source.ORG_KEY
			--,  _target.CLNT_ENGMT_CD=_source.CLNT_ENGMT_CD
			--,  _target.SECUR_SRVC_CD=_source.SECUR_SRVC_CD
			--,  _target.VULN_SRC_KEY=_source.VULN_SRC_KEY
			--,  _target.VULN_INSTC_STS_CD=_source.VULN_INSTC_STS_CD
			_target.VULN_SEV_CD=_source.VULN_SEV_CD
			,  _target.VULN_IMP_CD=_source.VULN_IMP_CD
			,  _target.RISK_PRBL_CD=_source.RISK_PRBL_CD
			,  _target.RMDTN_CST_EFFRT_CD=_source.RMDTN_CST_EFFRT_CD
			,  _target.ROOT_CAUS_ANLYS_CD=_source.ROOT_CAUS_ANLYS_CD
			,  _target.CVE_ID=_source.CVE_ID
			,  _target.OS_KEY=_source.OS_KEY
			--,  _target.SRC_VULN_SCAN_ID=_source.SRC_VULN_SCAN_ID
			,  _target.SRC_VULN_SCAN_STRT_DT=_source.SRC_VULN_SCAN_STRT_DT
			,  _target.SRC_VULN_SCAN_END_DT=_source.SRC_VULN_SCAN_END_DT
			--,  _target.SRC_VULN_INSTC_ID=_source.SRC_VULN_INSTC_ID
			,  _target.SRC_VULN_ID=_source.SRC_VULN_ID
			,  _target.VULN_NM=_source.VULN_NM
			,  _target.VULN_DESC=_source.VULN_DESC
			,  _target.VULN_CREAT_DT=_source.VULN_CREAT_DT
			,  _target.IPADR=_source.IPADR
			,  _target.PORT_NBR=_source.PORT_NBR
			,  _target.SRC_ADVS_TXT=_source.SRC_ADVS_TXT
			,  _target.SRC_VULN_BAS_SCOR = _source.SRC_VULN_BAS_SCOR
			,  _target.VULN_BAS_SCOR=_source.VULN_BAS_SCOR
			,  _target.VULN_IMP_SUB_SCOR=_source.VULN_IMP_SUB_SCOR
			,  _target.VULN_EXPLT_SUB_SCOR=_source.VULN_EXPLT_SUB_SCOR
			,  _target.VULN_TMPRL_SCOR=_source.VULN_TMPRL_SCOR
			,  _target.VULN_ENV_SCOR=_source.VULN_ENV_SCOR
			,  _target.VULN_VCTR_TXT=_source.VULN_VCTR_TXT
			,  _target.NTWK_NM=_source.NTWK_NM
			,  _target.PRTCL_NM=_source.PRTCL_NM
			,  _target.HST_NM=_source.HST_NM
			,  _target.DOM_NM=_source.DOM_NM
			,  _target.SFTW_NM=_source.SFTW_NM
			,  _target.OS_NM=_source.OS_NM
			,  _target.APPL_URL=_source.APPL_URL
			,  _target.NETBIOS_NM=_source.NETBIOS_NM
			,  _target.MAC_ADR_NM=_source.MAC_ADR_NM
			,  _target.VULN_TECH_COMMT_TXT=_source.VULN_TECH_COMMT_TXT
			,  _target.VULN_IMP_COMMT_TXT=_source.VULN_IMP_COMMT_TXT
			,  _target.RECOM_COMMT_TXT=_source.RECOM_COMMT_TXT
			,  _target.ROOT_CAUS_COMMT_TXT=_source.ROOT_CAUS_COMMT_TXT
			--,  _target.APPL_FL_UPLOAD_LOG_KEY=_source.APPL_FL_UPLOAD_LOG_KEY
			,  _target.UPDT_DT=GETDATE()
			,  _target.UPDT_USER_ID='2'
		...
		--	_target.UPDT_DT = GETDATE()
		--	,_target.UPDT_USER_ID = '2'
		--	,_target.ROW_STS_KEY = (SELECT MSTR_LKP_KEY FROM dbo.MSTR_LKP WHERE LKP_ENTY_TYP_NM = 'Row Status' and LKP_ENTY_NM = 'Deactive')
		OUTPUT $ACTION INTO @SummaryOfChanges
		OPTION(RECOMPILE);

		SELECT @INSERT_COUNTS = COUNT(*) FROM @SummaryOfChanges WHERE Change = 'INSERT'
		PRINT N'Message: Total ' + CAST(@INSERT_COUNTS AS VARCHAR) + N' VULN INSERTED.'	

		SELECT @UPDATE_COUNTS = COUNT(*) FROM @SummaryOfChanges WHERE Change = 'UPDATE'
		PRINT N'Message: TOTAL ' + CAST(@UPDATE_COUNTS AS VARCHAR) + N' VULN UPDATED.'				

		SELECT @DELETE_COUNTS = COUNT(*) FROM @SummaryOfChanges WHERE Change = 'DELETE'
...
END
GO
PRINT N'Creating [dbo].[MERGE_CLNT_VULN_INSTC_CISCO_SECUR_RISK_ASES]...';
...
/*
EXEC [dbo].[MERGE_CLNT_VULN_INSTC_CISCO_SECUR_RISK_ASES] ? OUTPUT , ? OUTPUT , ? OUTPUT , ? OUTPUT
...
CREATE PROCEDURE [dbo].[MERGE_CLNT_VULN_INSTC_CISCO_SECUR_RISK_ASES]
  @SRC_COUNTS  INT OUTPUT,
  @INSERT_COUNTS INT OUTPUT,
  @UPDATE_COUNTS INT OUTPUT,
  @DELETE_COUNTS INT OUTPUT
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT
			...
			,riskases_stg.RISK_DESC AS VULN_DESC
			...
			,NULL AS NETBIOS_NM
			,NULL AS MAC_ADR_NM
			,riskases_stg.CTL_DEFICIENCIES AS VULN_TECH_COMMT_TXT
			,riskases_stg.IMP_COMMT AS VULN_IMP_COMMT_TXT
			,riskases_stg.RMDTN_RECOM AS RECOM_COMMT_TXT
			,NULL AS ROOT_CAUS_COMMT_TXT
			,riskases_stg.APPL_FL_UPLOAD_LOG_KEY AS APPL_FL_UPLOAD_LOG_KEY

		FROM stg.CISCO_SECUR_RISK_ASES_STG riskases_stg
		INNER JOIN dbo.APPL_FL_UPLOAD_LOG fl_upd_log
		...
		/*
		--VALIDATIONS --DELETE
		SELECT * FROM stg.CISCO_SECUR_RISK_ASES_STG
		SELECT DISTINCT CLNT_ENGMT_CD FROM #Vulnerability_SecurRskAses
		SELECT * FROM #Vulnerability_SecurRskAses WHERE CLNT_ENGMT_CD = 'QCL-TR-20160628' AND SRC_VULN_ID = '18e23d56-c966-11e5-939a-00155d700b07'
		SELECT * FROM #Vulnerability_SecurRskAses WHERE CLNT_ENGMT_CD = 'QCL-TR-20160629'
		...
		SELECT * FROM #Vulnerability_SecurRskAses
		SELECT * FROM [dbo].[CVE] WHERE CVE_ID = 'CVE-1999-0001'
		*/
...
END
GO
PRINT N'Creating [dbo].[MERGE_CLNT_VULN_SECUR_CTL]...';
...
/*
EXEC [dbo].[MERGE_CLNT_VULN_SECUR_CTL]
...
CREATE PROCEDURE [dbo].[MERGE_CLNT_VULN_SECUR_CTL]
AS
BEGIN
	DECLARE
			@ROWS INT = 0
	...
	BEGIN TRY
		...
		FROM [dbo].[CLNT_VULN_INSTC] _clientvuln
		INNER JOIN [dbo].[CVE] _cve
			ON _clientvuln.[CVE_ID] = _cve.[CVE_ID]
		INNER JOIN [dbo].[CVE_SECUR_CTL_MAP] _cvescrtymap
			ON _cvescrtymap.[CVE_ID] = _cve.[CVE_ID]
		CROSS APPLY dbo.fnGetSecurCtlMap(_cvescrtymap.[REG_CMPLN_CD], _cvescrtymap.[REG_CMPLN_VER],_cvescrtymap.[SECUR_CTL_CD]) SecurCtldata
		WHERE NOT EXISTS
		(
			SELECT TOP 1 _clientvulnsecurctl.CLNT_VULN_INSTC_KEY
			FROM dbo.[CLNT_VULN_SECUR_CTL] _clientvulnsecurctl
			WHERE _clientvuln.CLNT_VULN_INSTC_KEY = _clientvulnsecurctl.CLNT_VULN_INSTC_KEY
		)

		/*
		SELECT * FROM #CLNT_VULN_SECUR_CTL
		SELECT * FROM [dbo].[CLNT_VULN_SECUR_CTL]

		SELECT * FROM [dbo].[CLNT_VULN_SECUR_CTL] _a
			INNER JOIN #CLNT_VULN_SECUR_CTL _b
			ON _a.CLNT_VULN_INSTC_KEY = _b.CLNT_VULN_INSTC_KEY
		*/
		...
		WHEN MATCHED AND 
			(			
			 _target.REG_CMPLN_CD=_source.REG_CMPLN_CD
			 AND _target.REG_CMPLN_VER=_source.REG_CMPLN_VER
			 AND _target.SECUR_CTL_CD=_source.SECUR_CTL_CD
			)
		THEN UPDATE SET 
			_target.ROW_STS_KEY= (SELECT MSTR_LKP_KEY FROM dbo.MSTR_LKP WHERE LKP_ENTY_TYP_NM = 'Row Status' and LKP_ENTY_NM = 'Active')
			,  _target.UPDT_DT=GETDATE()
			,  _target.UPDT_USER_ID='2'
		...
		SELECT @UPDATE_COUNTS = COUNT(*) FROM @SummaryOfChanges WHERE Change = 'UPDATE'
		PRINT N'TOTAL ' + CAST(@UPDATE_COUNTS AS VARCHAR) + N' VULN UPDATED.'				

		SELECT @INSERT_COUNTS = COUNT(*) FROM @SummaryOfChanges WHERE Change = 'INSERT'
		PRINT N'Total ' + CAST(@INSERT_COUNTS AS VARCHAR) + N' VULN INSERTED.'				

		SELECT @DELETE_COUNTS = COUNT(*) FROM @SummaryOfChanges WHERE Change = 'DELETE'
		PRINT N'Total ' + CAST(@DELETE_COUNTS AS VARCHAR) + N' VULN INSERTED.'				
...
BEGIN
       SELECT        DISTINCT H.ORG_NM [Client NAME],B.CLNT_ORG_KEY, A.CLNT_ENGMT_CD,B.CLNT_ENGMT_NM,
                           K.RPT_NM,J.FL_NM,CONVERT(VARCHAR(20),J.UPDT_DT,101) UPDT_DT,J.RPT_STS_KEY,I.LKP_ENTY_NM
       FROM          CLNT_SECUR_SRVC_ENGMT                    A
       JOIN          CLNT_ENGMT                                      B
       ON                   A.CLNT_ENGMT_CD                          =      B.CLNT_ENGMT_CD
       JOIN          USER_CLNT_SRVC_ASGN                      G
       ...
       AND                  CONVERT(VARCHAR(20),J.UPDT_DT,101) = CASE WHEN @UPDT_DT  = '' THEN CONVERT(VARCHAR(20),J.UPDT_DT,101)  ELSE @UPDT_DT END
...
/******************************
	** File: IRMaaSAdmin.SQL   
	** Name: [Report_ListReportNames]
	** Desc: This procedure Get data into RPT_NM table
	** Auth: Prasad varma
	** Date: 29/06/2016
	**************************
	** Change History
...
/******************************
	** File: IRMaaSAdmin.SQL   
	** Name: Reports_INSReportFileUploadLog
	** Desc: This procedure insert data into RPT_FL_UPLOAD_LOG table
	** Auth: Prasad varma
	** Date: 29/06/2016
	**************************
	** Change History
	...
	*******************************/
	...
	BEGIN
		...
		SET NOCOUNT ON
			...
			INSERT RPT_FL_UPLOAD_LOG(ROW_STS_KEY,ORG_KEY,CLNT_ENGMT_CD,RPT_NM_KEY,RPT_STS_KEY,FL_NM,FL_FLDR_PTH,CREAT_USER_ID,CREAT_DT ) VALUES
			(@ROW_STS_KEY,@ORG_KEY,@CLNT_ENGMT_CD,@RPT_NM_KEY,@RPT_STS_KEY,@FL_NM,@FL_FLDR_PTH,@USER_ID,GETDATE())
...
/******************************
	** File: IRMaaSAdmin.SQL   
	** Name: [Reports_UpdateReportFileUploadLog]
	** Desc: This procedure UPDATE and delete data into RPT_FL_UPLOAD_LOG table
	** Auth: Prasad varma
	** Date: 29/06/2016
	**************************
	** Change History
	...
	*******************************/
	...
	@FL_FLDR_PTH		  VARCHAR(500),
	@USER_ID			  INTEGER,
	@Flag				  VARCHAR(1) -- Flag U Update and Flag D Delete
	...
	BEGIN
		...
		SET NOCOUNT ON
			...
			SET		ROW_STS_KEY		=		@ROW_STS_KEY,
					...
					RPT_NM_KEY		=		@RPT_NM_KEY,
					RPT_STS_KEY		=		@RPT_STS_KEY,
					FL_NM			=		@FL_NM,
					FL_FLDR_PTH		=		@FL_FLDR_PTH,
					UPDT_USER_ID	=		@USER_ID,
					UPDT_DT			=		GETDATE()
...
END
GO
PRINT N'Creating [dbo].[TEST_CVSSScoreExample]...';
...
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (45, N'Service Status', N'Not Reviewed', N'Default Status for a service in analyst module', 1, CAST(0x0000A6270093B476 AS DateTime), 1, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (46, N'Service Status', N'Reviewed', N'Once a service reviewed it will be in Reviewed status', 1, CAST(0x0000A6270093B476 AS DateTime), 1, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (47, N'Report Status', N'Published', N'Status will be updated to Published after reports are reviewed', 1, CAST(0x0000A6270093B476 AS DateTime), 1, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (48, N'Report Status', N'Not Published', N'Default status for files uploaded in Reporter module', 1, CAST(0x0000A6270093B476 AS DateTime), 1, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (49, N'CVE_FILE', N'Modified', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-Modified.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (50, N'CVE_FILE', N'2002', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-2002.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (51, N'CVE_FILE', N'2003', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-2003.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (52, N'CVE_FILE', N'2004', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-2004.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (53, N'CVE_FILE', N'2005', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-2005.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (54, N'CVE_FILE', N'2006', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-2006.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (55, N'CVE_FILE', N'2007', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-2007.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (56, N'CVE_FILE', N'2008', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-2008.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (57, N'CVE_FILE', N'2009', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-2009.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (58, N'CVE_FILE', N'2010', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-2010.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (59, N'CVE_FILE', N'2011', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-2011.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (60, N'CVE_FILE', N'2012', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-2012.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (61, N'CVE_FILE', N'2013', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-2013.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (62, N'CVE_FILE', N'2014', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-2014.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (63, N'CVE_FILE', N'2015', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-2015.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
INSERT [dbo].[MSTR_LKP] ([MSTR_LKP_KEY], [LKP_ENTY_TYP_NM], [LKP_ENTY_NM], [LKP_ENTY_DESC], [ACTV_IND], [CREAT_DT], [CREAT_USER_ID], [UPDT_DT], [UPDT_USER_ID]) VALUES (64, N'CVE_FILE', N'2016', N'https://nvd.nist.v/feeds/xml/cve/nvdcve-2.0-2016.xml.zip', 1, CAST(0x0000A62700EDC9F4 AS DateTime), 2, NULL, NULL)
