CREATE VIEW [dbo].[Findings] AS
		SELECT  DISTINCT A.CLNT_VULN_INSTC_KEY,A.CLNT_ENGMT_CD,A.VULN_NM,A.VULN_SRC_KEY,B.LKP_ENTY_NM VULN_SRC,A.RMDTN_CST_EFFRT_CD,F.RMDTN_CST_EFFRT_NM,
					...
					A.SRC_VULN_SCAN_ID,CONVERT(VARCHAR,A.SRC_VULN_SCAN_STRT_DT,101)STRT_DT,
					CONVERT(VARCHAR,A.SRC_VULN_SCAN_END_DT,101)END_DT,A.SFTW_NM,A.HST_NM,
					ISNULL(A.APPL_URL,'')APPL_URL,E.OS_NM,A.NETBIOS_NM,H.VULN_IMP_CD,
					...
					A.RISK_PRBL_CD,I.RISK_PRBL_NM,A.ROOT_CAUS_ANLYS_CD,L.ROOT_CAUS_ANLYS_NM [RootCauseName],A.OWASP_TOP_10_KEY,
					M.OWASP_NM,A.SECUR_SRVC_CD,
					HITRUST = ISNULL(STUFF((
          SELECT DISTINCT ',' + D.SECUR_CTL_CD
          FROM   CLNT_VULN_SECUR_CTL D
		  JOIN	 SECUR_CTL			 E
          ...
          FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 1, ''),'')
			...
			LEFT JOIN   ROOT_CAUS_ANLYS						L
			ON		L.ROOT_CAUS_ANLYS_CD	=  A.ROOT_CAUS_ANLYS_CD
